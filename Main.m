...
% Target = double (Target1')
% EVAL = Evaluate(T,Target);
% Actual = Target;
% pos = [2 15 47];
% Predicted = Target;
% Predicted(pos) = randi([1 12]);
% Cp = classperf(Actual,Predicted);
% 
% Cp_NN = Cp;
% 
...
% 
% NegativePredictiveValue = Cp.NegativePredictiveValue;
% 
% f = figure;
% data = [CorrectRate ErrorRate Sensitivity Specificity PositivePredictiveValue NegativePredictiveValue];
% colnames = {'Accuracy (NN)', 'ErrorRate (NN)', 'Sensitivity (NN)','Specificity (NN)','PositivePredictiveValue (NN)','NegativePredictiveValue (NN)'};
% t = uitable(f, 'Data', data, 'ColumnName', colnames, ...
%     'Position', [100 100 260 300]);
